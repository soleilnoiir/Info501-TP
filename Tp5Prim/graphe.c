...
void afficherAreteKruskal(graphe_t g){
    int poid =0;
    arete_t* resultatKruskal = Kruskal(g);
    int i;

    for(i = 0; i <  g.nSommets-1; i++)
    {
        poid += resultatKruskal[i].poids;

    }
    printf("\nL'arbre couvrant de poid minimal Kruskall prend ce ");
    afficherArete(resultatKruskal,g.nSommets-1);
    printf("\nPour un poid total  de %d \n", poid);

}



void afficheraretePrim(graphe_t g){
  int i;
  int poid;
  ...
  for(i = 0; i < g.nSommets ; i++)
  {
      ...
      if(g.tSommets[i].pere != NULL)
      {
          printf("sommet : %d, distance : %d \n pere : %d \n\n",i,g.tSommets[i].distance,g.tSommets[i].pere->valeur);
      }
      else
      {
          printf("sommet : %d, distance : %d \n pere : origine \n\n",i,g.tSommets[i].distance);
      }
  }
...
void generer_acpm_prim_tableau(graphe_t G, int origine)
{
    int i;
    int j;
    int tmp = origine;
    ...
    for( i = 0 ; i < G.nSommets ; i++)
    {
        G.tSommets[i].distance = INT_MAX;
        G.tSommets[i].pere = NULL;
        G.tSommets[i].couvert = 0;
    }
    G.tSommets[origine].distance = 0;
    


    for(i = 0; i < G.nSommets ; i++)
    {
        G.tAretes[tmp].sommetOrigine;
        cell = G.ladj[tmp]->tete;
        G.tSommets[tmp].couvert = 1;
        min = INT_MAX;
        while(cell != NULL)
        {
            if(cell->poids < G.tSommets[cell->valeur].distance)
            {
                G.tSommets[cell->valeur].distance = cell->poids;
                G.tSommets[cell->valeur].pere = &G.tSommets[tmp];
            }
            if(G.tSommets[cell->valeur].distance < min && (G.tSommets[cell->valeur].couvert == 0))
            {
                min = G.tSommets[cell->valeur].distance;
                id = cell->valeur;
            }
            cell = cell->succ;
        }
...
// c'est dla merde prim tableau la


void generer_acpm_prim(graphe_t G, int origine){
