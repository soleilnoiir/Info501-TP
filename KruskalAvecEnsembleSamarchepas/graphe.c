...
liste_t** creerListesAdjacences(char* fileName){
    ...
    while( strcmp("FIN_DEF_ARETES", str ) != 0 ){
        ...
        if(strcmp("FIN_DEF_ARETES", str) != 0){
            ...
            inserer(liste[atoi(str)], initialiserCellule(str2, atoi(poids),NULL));
            inserer(liste[str2], initialiserCellule(atoi(str), atoi(poids),NULL));
        }
    }
    fclose(file);
    return liste;
...
arete_t *generer_acpm_kruskal_ensembles(graphe_t *g)
{
  recup_arete(g);
  tri_par_tas(g);
  ensemble_t *ens;
  ...
  arete_t *arbre_couvrant;
  arbre_couvrant = (arete_t *)malloc(sizeof(arete_t) * (g->nSommets - 1));
  int iterator = 0;
  ...
  {
    if (trouver_ensemble(ens, g->tAretes[i].sommetOrigine ) != trouver_ensemble(ens, g->tAretes[i].sommetExtremite ))
    {
      arbre_couvrant[iterator] = g->tAretes[i];
      iterator++;
      union_(ens, g->tAretes[i].sommetOrigine, g->tAretes[i].sommetExtremite);
    }
  }
